...
nasm -f bin -o "${OUTDATE}.img" "${OUTDATE}.test.asm"

echo "Booting QEMU to test instruction.  Press CTRL-A + X to break out of QEMU..."
qemu-system-i386 -boot a -fda $OUTDATE.img -nographic

echo "Created ${OUTDATE}.img and ${OUTDATE}.test.asm.  Do you want to keep these files?"